...
# 현재 재고가 바닥나는 시점 이전까지 받을 수 있는 라면 중 제일 많은 라면을 받는게 목표
# 현재 재고의 상태에 따라 최고값을 받아야함 동적으로 변경되는 상태 -> 제일 많은 값만 가져가면 됨
# 데이터를 넣을때마다 최대값을 동적으로 변경시키며 최소/최대값을 바로 꺼낼 수 있는 자료 구조 => Heap
